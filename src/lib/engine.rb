    ...
    def run_deploy_hook(index, cmd, cuid, muid, type, logger)
      begin
        Timeout::timeout(payload[:deploy_hook_timeout] || 60) do
          ...
          logger.puts("Finished: #{cmd}", Logger::INFO, "#{cuid}.#{muid}[#{type}#{index + 1}]")
        end
      rescue Hookit::Error::UnexpectedExit
        logger.puts("There was an unexpected exit from the deploy hook", Logger::ERROR, "#{cuid}.#{muid}[#{type}#{index + 1}]")
      rescue Timeout::Error
        logger.puts("The hook took longer than expected to run", Logger::ERROR, "#{cuid}.#{muid}[#{type}#{index + 1}]")
      end
    end
